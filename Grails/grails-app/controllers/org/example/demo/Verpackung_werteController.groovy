...
class Verpackung_werteController {
	...
	def index() {
		...
		def objCount
		if(params.q) {
			objList = Verpackung_werte.search(params.q + "*").results
			objCount = objList.size()
		}
		else {
			objList = Verpackung_werte.list(params)
			objCount = Verpackung_werte.count()
		}
    ...
    def create() {
		...
		case 'GET':
        	[objInstance: new Verpackung_werte(params)]
			
			break
		case 'POST':
	        def objInstance = new Verpackung_werte(params)
    ...
    def show() {
        def objInstance = Verpackung_werte.get(params.id)
        if (!objInstance) {
